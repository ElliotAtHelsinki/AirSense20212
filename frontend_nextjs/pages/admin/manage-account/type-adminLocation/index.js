...
import AdminLocation from "../../../../src/models/AdminLocation";
...
function Admin({ ...props }) {
    ...
    const { configTable, page, pageSize, refetch, onChangeOneParam } =
        usePagination(
            (params) => AdminLocation.search(params),
            ["role_name", "is_active"],
            {
                email: searchEmail,
    ...
    const ToggleActiveMutation = useMutation(
        "toggleActive",
        (id) => AdminLocation.toggleActive(id),
        {
            onSuccess: async () => {
                refetch();
            },
        ...
        },
        {
            ...
            render: (value, record) => (
                <div className='flex gap-4'>
                    {CAN_DETAIL_ADMIN && (
                        <Link
                            href={`/admin/manage-account/type-adminLocation/detail/${record.id}`}
                            passHref
                        >
                    ...
                    {CAN_UPDATE_ADMIN && (
                        <Link
                            href={`/admin/manage-account/type-adminLocation/update/${record.id}`}
                            passHref
                        >
    ...
    return (
        <section>
            ...
            <PageAdminLayout pageName='Danh sách quản trị viên trạm'>
                <div className='flex justify-between gap-8'>
                    ...
                    {CAN_CREATE_ADMIN && (
                        <Button type='primary'>
                            <Link href='/admin/manage-account/type-adminLocation/create'>
                                Thêm mới
                            </Link>
                        </Button>
                    )}
