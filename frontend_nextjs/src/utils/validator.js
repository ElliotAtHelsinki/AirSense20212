...
export function validateRequireComboboxAcceptZero(message) {
    return {
        required: true,
        validator: (value) => {
...
/**
 * validate bat buoc nhap input dua theo dieu kien required
 * @param msg : message loi
 * @param required: true, false
 * @returns {{validator: validator, required: *}}
 */
export function validateDynamicRequireInput(msg, required) {
    return {
        required,
        message: msg || "Vui lòng không bỏ trống mục này",
    };
}
    ...
    return {
        validator: (_, url) => {
            if (url && trim(url)) {
                ...
                const isLocalOrAddress =
                    /^(localhost|(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))(:([0-9]|[1-9][0-9]|[1-9][0-9]{2}|[1-9][0-9]{3}|[1-5][0-9]{4}|6([0-4][0-9]{3}|5([0-4][0-9]{2}|5([0-2][0-9]|3[0-5])))))?(\/?.*)?$/gi.test(
                        url
                    );
                if ((!isURL && !isLocalOrAddress) || haveWhitespace) {
                    return Promise.reject(
                        new Error(message) ||
                        "Sai định dạng URL. Ví dụ: https://example.com"
                    );
                }
                return Promise.resolve();
            }
    ...
    return {
        validator: (_, number) => {
            if (number && trim(number)) {
                const regex = /^(0)+[0-9]{0,10}$/g;
                const regexOffice = /^(0)+[0-9]{1,10}$/g;
                const regexHotline = /^(1)+[0-9]{0,7}$/g;
                ...
                if (
                    regex.test(number) &&
                    number.length <= 11 &&
                    (type !== "supportPhone" ? number.length >= 10 : true)
                ) {
                    isRegex = true;
                }
                if (type === "platformHotline" && regexHotline.test(number)) {
                    isRegex = true;
                }
                if (type === "office" || type === "supportPhone") {
                    if (
                        regexOffice.test(number) &&
                        number.length <= 11 &&
                        (type !== "supportPhone" ? number.length >= 10 : true)
                    ) {
                        isRegexOffice = true;
...
export function validatePhoneNumber2(message) {
    return {
        validator: (_, number) => {
            if (number && trim(number)) {
                const isNumber = isNumeric(number);
                ...
                if (
                    (phoneReplace.startsWith("1800") ||
                        phoneReplace.startsWith("1900")) &&
                    phoneReplace.length !== 8 &&
                    phoneReplace.length < 10
                ) {
    ...
    return {
        validator: (_, value) => {
            if (trim(value)) {
                ...
                return Promise.reject(
                    message || "Vui lòng nhập vào giá trị lớn hơn 0"
                );
            }
            return Promise.resolve();
        },
...
// Validate truong phai la so
export function validateNumberInput(message) {
    const regex = /^[0-9]+$/g;
    return {
        validator: (_, value) => {
            if (trim(value)) {
    ...
    return {
        validator: (_, value) => {
            if (value) {
                ...
                return Promise.reject(
                    message || "Nhập vượt quá chữ số cho phép"
                );
            }
            return Promise.resolve();
        },
...
export function validateMaxArraySelect(max, message) {
    return {
        validator: (_, values) => {
            ...
            return Promise.reject(
                message || `Vượt quá tối đa ${max} lựa chọn!`
            );
        },
    };
}
    ...
    return {
        validator: (_, values) => {
            if (Array.isArray(values) && values.length) {
                ...
                return Promise.reject(
                    message || `Mỗi lựa chọn nhập tối đa ${max} ký tự!`
                );
            }
            return Promise.resolve();
        },
    ...
    return {
        validator: (_, array) => {
            if (array) {
                ...
                if (!isError) return Promise.resolve();
                return Promise.reject(
                    message || `Ký tự ${charVal} không được phép sử dụng`
                );
            }
            return Promise.resolve();
        },
...
export function formatNormalizeNumberOtherZero(value, type) {
    if (trim(value)) {
        ...
        if (value !== "" && value !== "0") {
            return type === "normal"
                ? parseInt(value, 10)
                : parseInt(value, 10).toLocaleString("fr");
        }
        return "";
    }
    return "";
...
export function formatNormalizeTaxCode(value) {
    if (trim(value)) {
        const taxCode = value.toString().replaceAll(/\s/g, "");
        // let taxCode = '';
...
export function formatNormalizeTaxCodev2(value = "") {
    if (value && trim(value)) {
        const taxCode = value.toString().trim(); // .replaceAll(/\D/g, '');
...
export function formatNormalizeSlug(value) {
    if (value && trim(value)) {
        const taxCode = DX.stringToSlug(value);
        return taxCode;
    }
    return null;
}
...
export function validateStrLength(max1, max2, message) {
    return {
        validator: (_, value) => {
            value = value || "";
            if (
                (value && (value.length === max1 || value.length === max2)) ||
                value.length === 0
            ) {
                return Promise.resolve();
            }
            return Promise.reject(
                message || "Vui lòng chỉ nhập tối đa số ký tự cho phép"
            );
        },
    };
}
    ...
    return {
        validator: (_, value) => {
            if (trim(value)) {
                ...
                return Promise.reject(
                    message || `Vui lòng nhập vào giá trị lớn hơn ${max1}`
                );
            }
            return Promise.resolve();
        },
...
// Validate truong password
export function validateCompareOldPassword(
    oldPassword,
    messageCompare,
    messagePattern
) {
        ...
        validator: (_, value) => {
            if (trim(value)) {
                ...
                if (type === "INCREASE") {
                    ...
                    return Promise.reject(
                        message ||
                        `Vui lòng nhập vào giá trị lớn hơn ${valueQuanlity}`
                    );
                }
                if (type === "DECREASE") {
                    if (parseInt(value, 10) < valueQuanlity) {
                    ...
                    return Promise.reject(
                        message ||
                        `Vui lòng nhập vào giá trị nhỏ hơn ${valueQuanlity}`
                    );
                }
            }
            return Promise.resolve();
...
const validateFileType = (value) => {
    ...
    for (let i = 0; i < value.length; i++) {
        if (
            !validateVideo(value[i]) &&
            !validateImg(value[i]) &&
            !validateFileUpload(value[i])
        ) {
        ...
        validator: (_, value) => {
            if (value) {
                ...
                if (!validateFileSize(value)) {
                    return Promise.reject(
                        message || "File không được quá 10MB"
                    );
                }
                if (isOverLimitSize(value)) {
                    return Promise.reject(
                        message ||
                        "Tổng dung lượng các file không được quá 100MB"
                    );
                }
                if (isOverFileNumber(value)) {
                    return Promise.reject(
                        message || "Chỉ chọn tối đa 20 files"
                    );
                }
    ...
    return {
        validator: (_, value) => {
            if (value) {
                if (
                    value.list.length === 0 &&
                    (value.type === "OPTION" || value.type === "PRODUCT")
                ) {
                    return Promise.reject(
                        message || "Vui lòng không bỏ trống mục này"
                    );
...
export function validateMaxValueArr(maxLength, message) {
    return {
        validator: (_, value) => {
            if (
                value &&
                Math.max(...value.split(",").filter((n) => !!n)) > maxLength
            ) {
                return Promise.reject(message);
            }
            return Promise.resolve();
...
// Validate truong chi nhap khoang trang
export function validateRequireSelect(message) {
    return {
        required: true,
        validator: (_, value) => {
            if (value !== null) {
...
export function formValidatorAfterSubmit(form, res) {
    try{
        if (res.error) {
            form.setFields([
                {...form.getFieldsValue()},
